...
# null_resourceでECSタスクのステータスをチェックして、RUNNING状態になるまで待機
resource "null_resource" "wait_for_task_running" {
  provisioner "local-exec" {
    command = <<EOT
      task_arn=$(aws ecs list-tasks --cluster ${aws_ecs_cluster.main.id} --service-name ${aws_ecs_service.app.name} --query "taskArns[0]" --output text)
      ...
      status=$(aws ecs describe-tasks --cluster ${aws_ecs_cluster.main.id} --tasks $task_arn --query "tasks[0].lastStatus" --output text)
...
data "external" "fetch_eni_public_ip" {
  program = ["bash", "./get_eni_public_ip.sh", aws_ecs_cluster.main.id, aws_ecs_service.app.name]

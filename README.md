...
1. Railsで最低限のHelloWorldアプリケーション環境を構築
2. アプリケーション環境の最低限のコンテナ化
   - どんな設定を書くにもDockerfileに書くことを強制し、Infrastracture as a codeを実現
3. 最低限のReactNativeWebによりフロントとバックを分ける
4. 最低限GraphQLの設定をしてフロントとバックの接合をさせる
5. 必要に応じて以下を設定
   - unicorn
   - dcron
   - circleCI
